...
class DoneCommand(BaseCommand):
    ...
    def perform(self, objects, *args, **kwargs):
        ...
        while True:
            try:
                selection = int(input_function('Input item number: '))
                break
            except ValueError:
                print('Bad input, try again.')

        if objects[selection].done == ' ':
           objects[selection].done = ' + '
           print('Item #{} is checked as done'.format(selection))
        else:
            objects[selection].done = ' '
            print('Item #{} is checked as undone'.format(selection))
...
class SortCommand(BaseCommand):
    ...
    def perform(self, objects, *args, **kwargs):
        ...
        print('Select type of sorting:')
        print('0 - by task type')
        print('1 - by execution status')
        
        input_function = get_input_function()
        selection = None
        while True:
            try:
                selection = int(input_function('Input sort type: '))
        ...
        if selection == 0:
            objects.sort(key = lambda x: type(x))
        elif selection == 1:
